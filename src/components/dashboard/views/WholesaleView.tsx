...
export function WholesaleView({ filters, isComparing }: CafeViewProps) {
  ...
  // Sync modal client filter with global client filter
  useEffect(() => {
  ...
  const { data: modalOverviewResponse } = useOverview(
    "cafe",
    { ...modalFilters, isWholesale: 1 as const },
    {
      enabled: isAnyOpen,
    },
  );
  ...
  const evolutionFilters = useMemo(() => {
    if (evolutionLimit > 12) return { ...filters, isWholesale: 1 as const };
    const end = filters.period.end;
    const start = subMonths(end, 11);
    start.setDate(1);
    return { ...filters, period: { start, end }, isWholesale: 1 as const };
  }, [filters, evolutionLimit]);

  const modalEvolutionFilters = useMemo(() => {
    if (evolutionLimit > 12)
      return { ...modalFilters, isWholesale: 1 as const };
    const end = modalFilters.period.end;
    const start = subMonths(end, 11);
    start.setDate(1);
    return { ...modalFilters, period: { start, end }, isWholesale: 1 as const };
  ...
  } = useOverview("cafe", { ...filters, isWholesale: 1 as const });
  const {
    data: evolutionResponse,
    isLoading: isLoadingEvolution,
    isFetching: isFetchingEvolution,
  } = useEvolution<CafeMonthData>("cafe", {
    ...evolutionFilters,
    isWholesale: 1 as const,
  });
  const {
    data: distributionResponse,
    isLoading: isLoadingDistribution,
    isFetching: isFetchingDistribution,
  } = useDistribution("cafe", { ...filters, isWholesale: 1 as const });
  const { data: productsResponse } = useProducts("cafe", {
    ...filters,
    isWholesale: 1 as const,
  });
  ...
  const { data: compareOverviewResponse } = useOverview(
    "cafe",
    { ...comparisonFilters, isWholesale: 1 as const },
    {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {renderProductView({
        ...
        filters: { ...filters, isWholesale: 1 as const },
      })}
